...
import { STATUS_UPDATE_TEAM_CONTENT } from "@/constants";
...
export const menuItems = (row) => {
    ...
    return (
         <Menu size="sm">
            ...
            <MenuItem disabled={status === 1} onClick={() => { handleOpen(); updateContent(STATUS_UPDATE_TEAM_CONTENT(row, 'active')) }}>Mark as active</MenuItem>
            <MenuItem disabled={status === 2} onClick={() => { handleOpen(); updateContent(STATUS_UPDATE_TEAM_CONTENT(row, 'inactive')) }}>Mark as inactive</MenuItem>
          </Menu>
    )
}